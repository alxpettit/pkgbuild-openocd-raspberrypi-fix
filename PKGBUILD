...
# Contributer: Ferdinand B <theferdi265 at gmail dot com>
...
package() {
  ...
  mv -v "${pkgdir}/usr/share/openocd"{,-raspberrypi}
  mv -v "${pkgdir}/usr/bin/openocd"{,-raspberrypi}
  mv -v "${pkgdir}/usr/share/man/man1/"{openocd.1,openocd-raspberrypi.1}
  # Note: this breaks INFO file, but I don't care that much TBH
  mv -v "${pkgdir}/usr/share/info/"{openocd.info,openocd-raspberrypi}
  mv -v "${pkgdir}/usr/share/info/"{openocd.info-1,openocd-raspberrypi-1}
  mv -v "${pkgdir}/usr/share/info/"{openocd.info-2,openocd-raspberrypi-2}


  install -Dm 644 contrib/60-openocd.rules "$pkgdir"/usr/lib/udev/rules.d/60-openocd-raspberrypi.rules
}
